...
log_file = os.path.join(os.path.expanduser("~"), "nvda_yt_downloader.log")
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def start_batch_download(self, playlist_url, is_audio, quality_str, items, playlist_title):
	...
	def _start_actual_download(self, d_id):
		...
		thread = threading.Thread(
			...
			args=(d_id, params['url'], params['is_audio'], params['quality_str'], params['start_time'], params['end_time'], params['playlist_mode'], params['playlist_items'], params['playlist_title'], params.get('known_title'))
		)
		thread.start()
	...
	def _run_download_thread(self, d_id, url, is_audio, quality_str, start_time, end_time, playlist_mode, playlist_items, playlist_title, known_title=None):
		try:
			# 1. Fetch Title
			...
			process = downloader.download_video_with_process(
				url, download_path, is_audio, quality_str, start_time, end_time, progress_hook, playlist_mode, playlist_items, playlist_title
			)
			...
			for line in process.stdout:
				...
				if "[download]" in line:
					...
					self._update_ui_status(d_id, "Converting to MP3...", None)
				elif "[Merger]" in line:
